...
class PaymentsService
{
    ...
    public function distributeDividend(User $user, Asset $resource, DateTime $exDate)
    {
        if (!$resource instanceof InvestiblaInterface) {
            throw new NonInvestibleResourceException();
        }

        $this->em->transactional(function (EntityManagerInterface $em) use ($user, $resource, $exDate) {
            $account = $resource->getAccount();
            $totalInvested = $resource->getTotalInvested()->getAmount();
            $amount = $account->getBalance() - $totalInvested;
            $equity = $resource->getLastEquity()->getPrice()->getAmount();
            $yield = $amount / $equity;
                ...
                if ($individualInvested > 0) {
                    ...
                    if (!$investorAccount) {
                        $investorAccount = $this
                            ->accountService
                            ->createUserAccount($investor->getUser());
                    }
                    ...
                    $this
                        ...
                        ->requestTransfer(
                            ...
                            $dividendAmount,
                            $note,
                            function (EntityManagerInterface $em, Transaction $transaction) use ($resource, $dividend, $note) {
    ...
    public function distributeProfit(User $user, Project $resource, DateTime $exDate)
    {
        if (!$resource instanceof InvestiblaInterface) {
            throw new NonInvestibleResourceException();
        }
        ...
        $this->em->transactional(function (EntityManagerInterface $em) use ($user, $resource, $exDate) {
            ...
            foreach ($investors as $investor) {
                ...
                /** @var Investment $investment */
                foreach ($investor->getInvestments() as $investment) {
                    if ($investment->getResource()->getId() === $resource->getId()) {
                        $individualInvested += $investment->getAmount();
                    }
                }
                ...
                if ($individualInvested > 0) {
                    ...
                    $investorProfit = $this->getInvestorProfit($individualInvested, $resource->getCompany());

                    $investmentPercentage = $individualInvested / $resource->getPurchasePrice()->getAmount();
                    $proportionalAmount = $profit->getAmount() * $investmentPercentage;
                    $userProfitAmount = $proportionalAmount * $investorProfit->getProfit();
                    ...
                    $this
                        ...
                        ->requestTransfer(
                            ...
                            $individualInvested,
                            $note,
                            function (EntityManagerInterface $em, Transaction $transaction) use ($resource, $capitalReturn, $note) {
                    ...
                    $this
                        ...
                        ->requestTransfer(
                            ...
                            $userProfitAmount,
                            $note,
                            function (EntityManagerInterface $em, Transaction $transaction) use ($resource, $profit, $note) {
    ...
    private function companyCommissionPayment(
        ...
        if (!$companyAccount) {
            $companyAccount = $this
                ->accountService
                ->createCompanyAccount($resource->getCompany());
    ...
    private function commissionPayment(
        ...
        $this
            ...
            ->requestTransfer(
                ...
                $amount * $fee,
                $note,
                function (EntityManagerInterface $em, Transaction $transaction) use ($resource, $amount, $note, $fee) {
    ...
    public function saveRecurringIncome(Resource $resource, RecurringIncome $recurring)
    {
        $recurring->setResource($resource);
        $this->saveRecurring($resource->getRecurringIncomes(), $recurring);
    }

    public function saveRecurringExpense(Resource $resource, RecurringExpense $recurring)
    {
        $recurring->setResource($resource);
        $this->saveRecurring($resource->getRecurringExpenses(), $recurring);
    }

    public function removeRecurringIncome(Resource $resource, RecurringIncome $recurring)
    {
        $recurring->setResource($resource);
        $this->removeRecurring($resource->getRecurringIncomes(), $recurring);
    }

    public function removeRecurringExpense(Resource $resource, RecurringExpense $recurring)
    {
        $recurring->setResource($resource);
        $this->removeRecurring($resource->getRecurringExpenses(), $recurring);
    }

    private function saveRecurring(Collection $recurringCollection, Recurring $recurring)
    {
    ...
    private function removeRecurring(Collection $recurringCollection, Recurring $recurring)
    {
        if ($recurringCollection->contains($recurring)) {
    ...
    private function updateYearlyProjection(Resource $resource, Collection $recurringCollection)
    {
        $totalYearlyProjection = $this->getYearlyProjection(...$recurringCollection);
        $resource
    ...
    public function addIncome(User $user, IncomePayment $payment, bool $process, callable $fn = null)
    {
        $this
            ->transactionService
            ->requestDeposit(
                ...
                $payment->getAmount(),
                $payment->getNote(),
                function (EntityManagerInterface $em, Transaction $transaction) use ($payment, $process, $fn) {
    ...
    public function addExpense(User $user, ExpensePayment $payment, bool $process, callable $fn = null)
    {
        $this
            ->transactionService
            ->requestWithdrawal(
    ...
    {
        $amount = $this->getYearlyProjection(...$resource->getRecurringIncomes());
    ...
    {
        $amount = $this->getYearlyProjection(...$resource->getRecurringExpenses());
    ...
    {
        $profit = $project->getSaleRealPrice() - $project->getPurchasePrice() - $project->getConstructionCost();
    ...
    private function getInvestorProfit($investment, Company $company)
    {
        ...
        $investorProfitRepository = $this->em->getRepository(InvestorProfit::class);
        $investorProfit = $investorProfitRepository->findByInvestmentValue($investment, $company);
